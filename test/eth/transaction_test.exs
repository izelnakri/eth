...
defmodule ETH.TransactionTest do
  ...
  test "hash/1 works" do
    ...
    assert ETH.Transaction.hash(first_transaction_list) ==
             decode16("375a8983c9fc56d7cfd118254a80a8d7403d590a6c9e105532b67aca1efb97aa")

    assert ETH.Transaction.hash(first_transaction_list, false) ==
             decode16("61e1ec33764304dddb55348e7883d4437426f44ab3ef65e6da1e025734c03ff0")

    assert ETH.Transaction.hash(first_transaction_list, true) ==
             decode16("375a8983c9fc56d7cfd118254a80a8d7403d590a6c9e105532b67aca1efb97aa")

    assert ETH.Transaction.hash(second_transaction_list) ==
             decode16("0f09dc98ea85b7872f4409131a790b91e7540953992886fc268b7ba5c96820e4")

    assert ETH.Transaction.hash(second_transaction_list, true) ==
             decode16("0f09dc98ea85b7872f4409131a790b91e7540953992886fc268b7ba5c96820e4")

    assert ETH.Transaction.hash(second_transaction_list, false) ==
  ...
  test "get_sender_address/1 works" do
    ...
    |> Enum.each(fn transaction ->
      transaction_list =
        transaction |> Map.get("raw") |> ETH.Transaction.parse() |> ETH.Transaction.to_list()
  ...
  test "send_transaction_works" do
    output =
      ...
      |> ETH.Transaction.sign_transaction(
